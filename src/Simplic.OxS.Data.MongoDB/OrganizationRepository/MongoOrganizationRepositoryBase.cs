    ...
    where TFilter : IOrganizationFilter<Guid>, new()
{
    ...
    /// <returns>Instance if document if found, else null</returns>
    public override async Task<TDocument> GetAsync(Guid id) => await GetAsync(id, false);
    ...
    /// <summary>
    /// Get all documents from the current organization / collection
    /// </summary>
    /// <returns>Enumerable of documents</returns>
    public async Task<IEnumerable<TDocument>> GetAllAsync() => await GetAllAsync(false);
    ...
    {
        var builder = Builders<TDocument>.Filter;
        var filterQueries = new List<FilterDefinition<TDocument>>
        {
        ...
        var filter = builder.And(filterQueries);

        return context.GetCollection<TDocument>(GetCollectionName()).Find(filter).AsExecutable();
    }
}
